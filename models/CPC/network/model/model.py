...
class CDCK6(nn.Module):
    ...
    def forward(self, x, x_reverse, hidden1, hidden2):
        ...
        t_samples = torch.randint(self.seq_len / 160 - self.timestep,
                                  size=(1,)).long()  # randomly pick time stamps. ONLY DO THIS ONCE FOR BOTH GRU.
...
class CDCK2(nn.Module):
    def __init__(self, timestep, batch_size, seq_len, input_channel=1, hidden_size=64):
        ...
        self.Wk = nn.ModuleList([nn.Linear(self.hidden_size//2, self.hidden_size) for _ in range(timestep)])
        self.softmax = nn.Softmax()
        self.lsoftmax = nn.LogSoftmax()
    ...
    def init_hidden(self, batch_size, use_gpu=True):
        if use_gpu:
            return torch.zeros(1, batch_size, self.hidden_size // 2).cuda()
        else:

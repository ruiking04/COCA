...
import os
import random
from utils.affiliation_metric import affiliation_accumulator, tsad_affiliation
from utils.reasonable_metric import reasonable_accumulator, tsad_reasonable
# os.environ['JAVA_HOME'] = '/opt/jdk-11.0.13'
# os.environ['CUDA_VISIBLE_DEVICES'] = '3'
os.environ['JAVA_HOME'] = '/mnt/nfsData/jdk'
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["SR"],
        help="Name of model (or models in ensemble) to benchmark. RRCF, SR",
...
def train_model(
    ...
    csv = os.path.join(dirname, f"pred_{dataset_name}_{rand}.csv.gz")
...
def read_model_predictions(dataset: TSADBaseDataset, model_dir: str):
    ...
    """
    csv = os.path.join("results", "anomaly", model_dir, f"pred_{dataset_to_name(dataset)}_{rand}.csv.gz")
...
def evaluate_predictions(
    ...
    scores_rpa, scores_pw, scores_pa, scores_reasonable, scores_affiliation = [], [], [], [], []
    ...
    with open('./log.txt', 'a') as f:
        if dataset_to_name(dataset) == 'UCR':
            score_reasonable_all = sum(scores_reasonable, reasonable_accumulator())
            print(
                f'{model_names} {dataset_to_name(dataset)} Reasonable Metrics',
                file=f
            )
            # print('>' * 32)
            print(
                f'{model_names} {dataset_to_name(dataset)} ',
                str(score_reasonable_all.get_all_metrics()),
                file=f
            )
            # print('>' * 32)
        ...
        if dataset_to_name(dataset) in {'UCR', 'IOpsCompetition'}:
            ...
            print(
                f'{model_names} {dataset_to_name(dataset)} Affiliation Metrics',
                file=f
            )
            # print('>' * 32)
            print(
                f'{model_names} {dataset_to_name(dataset)} ',
                str(score_affiliation_all.get_all_metrics()),
                file=f
            )
            # print('>' * 32)
...
def main():
    ...
    if args.visualize:
        ...
        model_name = "+".join(sorted(resolve_model_name(m) for m in args.models)) + '_' + str(rand)
